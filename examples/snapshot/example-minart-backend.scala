...
//> using lib "eu.joaocosta::interim::0.1.0-SNAPSHOT"
...
object MinartBackend:
  ...
  def run(body: InputState => (List[RenderOp], _)): Future[Unit] =
    AppLoop
      ...
      .configure(Canvas.Settings(width = 640, height = 480, title = "Immediate GUI", clearColor = MinartColor(255, 150, 100)), LoopFrequency.hz60)
      .run()
